...
export default function AdminDashboardPage() {
  ...
  return (
    <div>
      <h1 className="font-display text-2xl text-brand-ink font-bold mb-1">Dashboard</h1>
      <p className="text-brand-muted text-sm mb-8">Overview of everything happening across the ministry site.</p>

      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {cards.map(({ label, value, total, icon: Icon, to, urgent }) => (
          <Link key={label} to={to} className={`edge-card rounded-2xl p-5 ${urgent && value > 0 ? 'border-brand-sky/40' : ''}`}>
            <Icon className="text-brand-blue mb-3" size={20} />
            <div className="font-display text-2xl text-brand-ink font-bold">{value ?? '—'}{total !== undefined ? <span className="text-brand-muted/70 text-base"> / {total}</span> : ''}</div>
      ...
      {stats?.donationTotals && Object.keys(stats.donationTotals).length > 0 && (
        <div className="edge-card rounded-2xl p-6">
          <h2 className="font-display text-brand-ink font-semibold mb-4">Total Given (Successful)</h2>
          <div className="flex flex-wrap gap-6">
            {Object.entries(stats.donationTotals).map(([currency, amount]) => (
              <div key={currency}>
                <div className="font-display text-xl text-brand-blue font-bold">{currency} {Number(amount).toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
