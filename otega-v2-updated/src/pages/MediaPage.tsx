  ...
  return (
    <Section>
      ...
      <div className="flex items-end justify-between flex-wrap gap-4 mb-8">
        <h1 className="font-display text-4xl sm:text-5xl text-brand-ink font-bold">Media Gallery</h1>
        <div className="flex gap-2 bg-white p-1 rounded-full">
          {(['photos', 'videos'] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-5 py-2 rounded-full text-sm font-medium capitalize transition-colors ${tab === t ? 'bg-brand-blue text-white' : 'text-brand-muted'}`}>
              {t}
            </button>
          ))}
        </div>
            ...
            {videos.map((v) => (
              <div key={v.id} className="edge-card rounded-2xl overflow-hidden">
                {v.source === 'youtube' && v.youtube_id ? (
                  ...
                  <div className="aspect-video flex items-center justify-center text-brand-muted/40"><Play size={32} /></div>
                )}
                <div className="p-4">
                  <h3 className="font-display text-brand-ink font-semibold">{v.title}</h3>
                  {v.description && <p className="text-brand-muted text-sm mt-1 line-clamp-2">{v.description}</p>}
                </div>
              </div>
            ))}
          </div>
      ...
      {lightbox && (
        <div onClick={() => setLightbox(null)} className="fixed inset-0 bg-brand-milk/95 z-50 flex items-center justify-center p-6 cursor-zoom-out">
