...
export default function AdminSidebar({ user }: { user: AdminUser | null }) {
  ...
  return (
    <aside className="w-64 flex-shrink-0 bg-[#0A0A0A] border-r border-white/10 h-screen sticky top-0 flex flex-col">
      <div className="px-5 py-6 border-b border-white/10">
        <div className="font-display text-lg font-semibold text-white">Otega Admin</div>
        {user && <div className="text-white/40 text-xs mt-0.5">{user.displayName}</div>}
      </div>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                isActive ? 'bg-brand-lime text-black' : 'text-white/60 hover:text-white hover:bg-white/5'
              }`
            }
          >
            <Icon size={16} /> {label}
