  ...
  if (status === 'done') {
    return (
      <div className="edge-card rounded-2xl p-10 text-center">
        ...
        <h3 className="font-display text-xl text-white font-semibold">Your request has been received</h3>
        <p className="text-white/60 mt-2 text-sm">Our prayer team will be standing with you. Thank you for trusting us with this.</p>
        <button onClick={() => setStatus('idle')} className="mt-6 text-brand-lime text-sm font-medium">Submit another request</button>
      </div>
    );
  }
    ...
    <form onSubmit={onSubmit} className="edge-card rounded-2xl p-6 sm:p-8 space-y-5">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}
          className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-brand-lime">
      ...
      <p className="text-white/30 text-xs text-center">Your request is kept private and only seen by our prayer team.</p>
    </form>
  );
}
