...
export default function BlogPostPage() {
  ...
  return (
    <Section className="max-w-3xl mx-auto">
      <Link to="/blog" className="inline-flex items-center gap-1 text-white/50 text-sm mb-8 hover:text-white transition-colors">
      ...
      <h1 className="font-display text-3xl sm:text-5xl text-white font-bold mt-3 leading-tight">{post.title}</h1>
      <div className="flex items-center gap-3 text-white/40 text-sm mt-4">
        <span>{post.author}</span><span>·</span>
        <span>{new Date(post.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
      </div>
      <div className="prose prose-invert max-w-none mt-10">
        {post.content.split('\n').filter(Boolean).map((para, i) => (
          <p key={i} className="text-white/70 leading-relaxed text-lg mb-5">{para}</p>
        ))}
      </div>
    </Section>
  );
