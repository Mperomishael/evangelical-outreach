...
export default function AdminSettingsPage() {
  ...
  if (!settings) return <div className="flex justify-center py-20 text-white/40"><Loader2 className="animate-spin" size={24} /></div>;

  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        <h1 className="font-display text-2xl text-white font-bold">Settings</h1>
        ...
        {TABS.map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2.5 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${
              tab === t ? 'border-brand-lime text-white' : 'border-transparent text-white/50 hover:text-white'
            }`}>
            {t}
          </button>
        ))}
      ...
      {tab === 'Founder' && (
        <div className="space-y-4 max-w-xl">
          <p className="text-white/40 text-xs mb-2">Powers the dedicated /founder page.</p>
      ...
      {tab === 'Admin Users' && (
        currentUser?.isSuper ? <AdminUsersPanel /> : (
          <p className="text-white/50 text-sm">Only the super admin can manage other admin accounts.</p>
        )
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      <input value={value || ''} onChange={(e) => onChange(e.target.value)}
        className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm outline-none focus:border-brand-lime" />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <textarea rows={rows} value={value || ''} onChange={(e) => onChange(e.target.value)}
        className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm outline-none focus:border-brand-lime resize-none" />
    </div>
  );
}
  ...
  return (
    <div className="max-w-xl space-y-8">
      <form onSubmit={onCreate} className="edge-card rounded-2xl p-6 space-y-4">
        <h3 className="font-display text-white font-semibold flex items-center gap-2"><UserPlus size={18} /> New Sub-Admin</h3>
        <div className="grid sm:grid-cols-2 gap-3">
          <input required placeholder="Username" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })}
            className="bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm outline-none focus:border-brand-lime" />
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} className="border-t border-white/5">
                  ...
                  <td className="px-4 py-3 text-white/60">{u.display_name}</td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={async () => { await deleteAdminUser(u.id); refresh(); }} className="text-white/50 hover:text-red-400 p-1.5">
                      <Trash2 size={14} />
                    </button>
                  </td>
                </tr>
