  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center px-5">
      <form onSubmit={onSubmit} className="w-full max-w-sm edge-card rounded-2xl p-8">
        <div className="flex items-center gap-2 text-white font-display font-semibold text-lg mb-8">
          <Flame className="text-brand-lime" size={22} /> Otega Admin
        </div>
        <label className="block text-white/60 text-xs font-medium mb-1.5">Username</label>
        <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="superadmin"
          className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm outline-none focus:border-brand-lime mb-4" />
        <label className="block text-white/60 text-xs font-medium mb-1.5">Password</label>
        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••"
          className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm outline-none focus:border-brand-lime mb-6" />
        ...
        <p className="text-white/30 text-xs text-center mt-5">
          Super admin uses <span className="text-white/50">ADMIN_PASSWORD</span> only — leave username blank.
        </p>
      </form>
    </div>
  );
