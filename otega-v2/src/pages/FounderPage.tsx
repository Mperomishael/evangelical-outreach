    ...
    <>
      ...
      <section className="relative">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 pt-14 sm:pt-20 pb-16 grid md:grid-cols-12 gap-10 items-center">
          <div className="md:col-span-5">
            <div className="aspect-[4/5] rounded-3xl overflow-hidden bg-white/5 edge-card">
              {settings.founder_photo_url ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-white/20 font-display text-6xl">
          ...
          <div className="md:col-span-7">
            ...
            <h1 className="font-display text-4xl sm:text-6xl text-white font-bold leading-tight tracking-tight">
              {settings.founder_name || 'The Founder'}
            </h1>

            {settings.founder_quote && (
              <div className="mt-8 flex gap-4">
                <Quote className="text-brand-lime flex-shrink-0" size={28} />
                <p className="text-white/80 text-lg sm:text-xl leading-relaxed italic">{settings.founder_quote}</p>
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center gap-4 mt-8">
              ...
              {settings.contact_email && (
                <a href={`mailto:${settings.contact_email}`} className="border border-white/20 text-white px-6 py-3 rounded-full font-semibold flex items-center gap-2 hover:border-brand-lime transition-colors">
                  <Mail size={16} /> Email
                </a>
              )}
              {socials.map(({ url, Icon, label }) => (
                <a key={label} href={url!} target="_blank" rel="noreferrer" aria-label={label}
                   className="w-11 h-11 rounded-full border border-white/15 flex items-center justify-center hover:border-brand-lime hover:text-brand-lime transition-colors">
                  <Icon size={18} />
                </a>
              ))}
            </div>
        ...
        <Section className="border-t border-white/5">
          ...
          <div className="prose prose-invert max-w-3xl">
            {settings.founder_bio.split('\n').filter(Boolean).map((para, i) => (
              <p key={i} className="text-white/70 leading-relaxed text-base sm:text-lg mb-4">{para}</p>
            ))}
          </div>
        </Section>
      )}
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {settings.founder_vision && (
              <div className="edge-card rounded-2xl p-8">
                <h3 className="font-display text-2xl text-white font-semibold mb-3">Vision</h3>
                <p className="text-white/70 leading-relaxed">{settings.founder_vision}</p>
              </div>
            )}
            {settings.founder_mission && (
              <div className="edge-card rounded-2xl p-8">
                <h3 className="font-display text-2xl text-white font-semibold mb-3">Mission</h3>
                <p className="text-white/70 leading-relaxed">{settings.founder_mission}</p>
              </div>
            )}
          </div>
        </Section>
      ...
      <Section>
        <div className="edge-card rounded-3xl p-10 sm:p-16 text-center bg-gradient-to-br from-white/[0.03] to-transparent">
          <h2 className="font-display text-3xl sm:text-4xl text-white font-bold">Partner with {settings.founder_name || 'the ministry'}</h2>
          <p className="text-white/60 mt-3 max-w-lg mx-auto">Your support fuels evangelists on the field every day.</p>
